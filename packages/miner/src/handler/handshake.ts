...
const handshake = async (data: Buffer) => {
  ...
  const { value: serverAddressOffset, length: saLength } = readVarInt(restOfFields);
  const serverAddress = [...restOfFields.subarray(saLength, serverAddressOffset + saLength)]
